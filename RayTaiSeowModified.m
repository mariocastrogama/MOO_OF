function [f,g] = RayTaiSeowModified(x)
% RayTaiSeowModified.m
...
% requirements.
%
% In its modified version the 3 cost of investment are coupled into one single objective
% and the 2 cost of damages are coupled in another objective
%
% Objective Functions:  2, non-linear.
...
% As a matter of fact this problem can be seen as an early surrogate-model for
...
%
  ...
  cf = zeros(1,3);
  df = zeros(1,2);
  g(1) = ( 0.00139 /(x(1)*x(2)) +     4.940*x(3) -    0.0800 )/     1.0 - 1.0; % <=     1?  g(2) = ( 0.000306/(x(1)*x(2)) +     1.082*x(3) -    0.0986 )/     1.0; % <=     1?
  g(3) = (12.307   /(x(1)*x(2)) + 49408.240*x(3) + 4051.0200 )/ 50000.0 - 1.0; % <= 50000?
  g(4) = ( 2.098   /(x(1)*x(2)) +  8046.333*x(3) -  696.7100 )/ 16000.0 - 1.0; % <= 16000?
  g(5) = ( 2.138   /(x(1)*x(2)) +  7883.390*x(3) -  705.0400 )/ 10000.0 - 1.0; % <= 10000?
  g(6) = ( 0.417   /(x(1)*x(2)) +  1721.260*x(3) -  136.5400 )/  2000.0 - 1.0; % <=  2000?
  g(7) = ( 0.164   /(x(1)*x(2)) +   631.130*x(3) -   54.4800 )/   550.0 - 1.0; % <=   550?
  if any(g > 0);
    f = f + [10 30]; % Assign a value 
    nviolate = nviolate + 1;
  else
    cf(1) = 106780.37*(x(2)+x(3))+61704.67;               cf(1) = cf(1) ;%/ 8e4;
    cf(2) = 3000.0*x(1);                                  cf(2) = cf(2) ;%/ 1500.0;
    cf(3) = (305700.0)*2289.0*x(2)/(0.05*2289.0)^0.65;    cf(3) = cf(3) ;%/ 3e6;
    df(1) = (250.0)*2289*exp(-39.75*x(2)+9.90*x(3)+2.74); df(1) = df(1) ;%/ 6e6;
    df(2) = 25.0*(1.39/(x(1)*x(2)) + 4940*x(3) - 80.0);   df(2) = df(2) ;%/ 8e3;
    f(1)  = sum(cf)/1e6;
    f(2)  = sum(df)/1e6;
  end
  nfuneval = nfuneval + 1;
